"""
Unit tests for ru/notebooks/visualizations.py
...
def vis_module():
    ...
    spec = importlib.util.spec_from_file_location("visualizations_ru", module_path)
...
class TestBuildDemoDB:
    ...
    def test_categories_names_russian(self, db_connection):
        """Russian version uses Russian category names."""
        cur = db_connection.cursor()
    ...
    def test_suppliers_names_russian(self, db_connection):
        """Russian version uses Russian supplier prefix."""
        cur = db_connection.cursor()
        cur.execute("SELECT SupplierName FROM Suppliers LIMIT 1")
        name = cur.fetchone()[0]
        assert "\u041f\u043e\u0441\u0442\u0430\u0432\u0449\u0438\u043a" in name
    ...
    def test_products_name_russian(self, db_connection):
        """Russian version uses 'Товар' prefix for products."""
        cur = db_connection.cursor()
        cur.execute("SELECT ProductName FROM Products LIMIT 1")
        name = cur.fetchone()[0]
        assert "\u0422\u043e\u0432\u0430\u0440" in name
    ...
    def test_customers_name_russian(self, db_connection):
        """Russian version uses 'Клиент' prefix."""
        cur = db_connection.cursor()
        cur.execute("SELECT CustomerName FROM Customers LIMIT 1")
        name = cur.fetchone()[0]
        assert "\u041a\u043b\u0438\u0435\u043d\u0442" in name
    ...
    def test_employees_names_russian(self, db_connection):
        """Russian version uses Russian employee names."""
        cur = db_connection.cursor()
        cur.execute("SELECT FirstName FROM Employees WHERE EmployeeID = 1")
        name = cur.fetchone()[0]
        assert name == "\u0410\u043d\u043d\u0430"
...
class TestChartMonthlyTrend:
    ...
    def test_seasonal_pattern(self, db_connection):
        """December revenue should exceed January (seasonal weighting)."""
        q = """
            SELECT strftime('%m', o.OrderDate) AS month,
                   SUM(p.Price * od.Quantity)  AS revenue
            FROM Orders o
...
class TestMain:
    def test_main_runs_without_error(self, vis_module, tmp_results, capsys):
        ...
        assert "\u0413\u0440\u0430\u0444\u0438\u043a\u0438 \u0441\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u044b:" in captured.out
